	...
	<table>
		...
		<c:forEach items="${listEmp }" var="emp">
			<tr>
				...
				<td><a href="${pageContext.request.contextPath}/update.jsp?edit=false&&index=${count}&&currentUser=false"><button>View Details</button></a>
				</td>
				<c:if test="${currentUser.getPermission().equals('admin') }">
					<td><a href="${pageContext.request.contextPath}/update.jsp?edit=true&&index=${count}&&currentUser=false"><button>Update Details</button></a>
					</td>
					<td><a href="${pageContext.request.contextPath}/Delete?id=${emp.getId()}"><button>Delete</button></a>
					</td>
				</c:if>
			</tr>
